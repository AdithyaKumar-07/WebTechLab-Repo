...
function exportLog() {
  ...
  const header =
    "TIMESTAMP | EVENT | TARGET | PHASE | DATA\n" + "=".repeat(50) + "\n";
  const body = activityLog
    .map(
      (log) =>
        `${log.timestamp} | ${log.type.padEnd(8)} | ${log.target.padEnd(
          12
        )} | ${log.phase.padEnd(10)} | ${log.key || ""}`
    )
    .join("\n");
